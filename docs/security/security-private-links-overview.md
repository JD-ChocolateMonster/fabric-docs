...
* Connections to Fabric/Power BI data hosting solutions (Datamart, Data Warehouse, Lake House) from a Power BI Report's semantic model need an On-Premise or VNet Data Gateway to work when the tenant setting **Block Public Internet Access** is enabled in Fabric.
